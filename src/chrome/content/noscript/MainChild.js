var MainChild = {
  ...
  reload(browser, snapshots, mustReload, reloadPolicy, innerWindowID = 0) {
    ...
    let checkUnchangedForbiddenTop = sites.docSites.length === 1; // no frames
    if (checkUnchangedForbiddenTop) {
      // reorder to put top first
      let {topSite} = sites;
      allSites = allSites.filter(s => s !== topSite);
      allSites.unshift(topSite);
    }
    ...
    for (let site of allSites) {
      ...
      if (checkUnchangedForbiddenTop) {
        ...
        checkUnchangedForbiddenTop = false;
      }
    }
